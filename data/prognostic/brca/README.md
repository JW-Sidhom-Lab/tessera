...
| `../../TCGA_PanCan/ncit.csv` | TCGA Pan-Cancer Atlas curated clinical resource | Liu, J. *et al.* *Cell* **173**, 400-416.e11 (2018). |
...
`ncit.csv` is the standard TCGA Pan-Cancer Atlas clinical resource,
gitignored as a raw upstream input under
[`data/TCGA_PanCan/`](../../TCGA_PanCan/README.md). For BRCA we pull
the four standard endpoint pairs plus `ajcc_pathologic_tumor_stage`.
...
2. Loads `ncit.csv`, restricts to `type=='BRCA'`, normalises the AJCC
   stage strings (drops the A/B/C suffix; replaces `[Discrepancy]` /
   `[Not Available]` / `Stage X` with `NaN`), casts the four event
...
| `OS`, `OS.time`, `DSS`, `DSS.time`, `DFI`, `DFI.time`, `PFI`, `PFI.time` | TCGA standard survival endpoints. |
| `ajcc_pathologic_tumor_stage` | AJCC pathologic stage with A/B/C suffix removed. |
