"""Pretrain a TESSERA CNA-only model on the TCGA Pan-Cancer cohort.

Loads the TCGA training and validation CNA segment tables (produced by
``scripts/data/tcga/create_training_cna.py``) and trains a TESSERA CNA
encoder under one of three architecture configurations: 0, 1, or 2
inter-segment self-attention blocks (the ``attn_0`` / ``attn_1`` /
``attn_2`` models reported in Figure 2 of the manuscript).

Each model has a segment-mean reconstruction head plus an optional LOH
classification head. The segment-mean output is supervised with masked-
token regression; LOH is supervised when ``predict_cna_loh = True`` in
``model_config.py`` (the default; the manuscript also reports a NoLOH
variant for the cross-platform validation on MSK-CHORD where allele-
specific copy-number is unavailable).
...
Usage
-----
    python fit_model.py                              # defaults: attn_0
