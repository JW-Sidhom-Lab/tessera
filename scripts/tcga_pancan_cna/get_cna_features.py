"""Extract per-segment latent features from a trained TESSERA CNA model.

Loads a trained CNA model and saves per-segment latent features to
``cna_features/<model_name>_cna_features.pkl``. The output is a dict
with ``train_features_cna``, ``valid_features_cna``, plus the input
metadata (``train_data_cna``, ``valid_data_cna``) for downstream joins.
...
Usage
-----
    python get_cna_features.py                   # attn_0 (default)
