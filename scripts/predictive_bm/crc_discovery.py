...
# Per-learner config (validated CRC settings from prior coord descent)
PCA_VAR = 0.99
K_MU1, SP_MU1 = 2, 0.5
K_MU0, SP_MU0 = 2, 0.85
K_E,   SP_E   = 3, 0.0
...
def main():
    ...
    print(f"  arm counts: FOLFOX(arm=1)={arm_counts.get(1,0)}  FOLFIRI(arm=0)={arm_counts.get(0,0)}")
    ...
    print()
    print(f"  ABOVE PFS  n={a_pfs['n']:4d}   HR(FOLFOX vs FOLFIRI) = {a_pfs['HR']:.3f} "
          f"[{a_pfs['HR_lo']:.3f}, {a_pfs['HR_hi']:.3f}]   P = {a_pfs['P']:.3e}")
    print(f"  BELOW PFS  n={b_pfs['n']:4d}   HR(FOLFOX vs FOLFIRI) = {b_pfs['HR']:.3f} "
          f"[{b_pfs['HR_lo']:.3f}, {b_pfs['HR_hi']:.3f}]   P = {b_pfs['P']:.3e}")
    print(f"  ABOVE OS   n={a_os['n']:4d}   HR(FOLFOX vs FOLFIRI) = {a_os['HR']:.3f} "
