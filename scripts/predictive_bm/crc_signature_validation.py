"""Per-signature predictive-biomarker validation for the CRC PMD decomposition.

For each PMD signature k we compute a per-patient score
    s_pk = u_pk × D_k × sum_g v_gk
which is the patient's contribution to τ̂_p from signature k (same units as τ̂).
We then run the same interaction-Cox + indifference-threshold + per-stratum KM
analysis as the main Fig. 6 pipeline, INDIVIDUALLY for each signature.

A signature passes the predictive-biomarker test if:
  - PFS interaction P < 0.05 (treatment-by-signature interaction is real)
  - Above-τ̂0_k stratum and below-τ̂0_k stratum have direction-reversed arm HRs
...
def _safe_arm_hr(df, mask, *, time_col, event_col):
    """Wrapper that returns NaN-filled dict on degenerate (single-arm or no-event) strata."""
    sub = df[mask]
...
def validate_signature(df: pd.DataFrame, score: np.ndarray, *, label: str) -> dict:
    """Run interaction Cox + threshold + per-stratum HRs for one signature."""
    work = df.copy()
    # Sign-align so larger score = arm-1 favored
...
def km_panel(ax, df, mask, *, time_col, event_col, label, hr, p, n,
              ...
              xlim=24.0):
    """One KM panel: within-stratum, color by arm."""
    sub = df[mask]
...
def main():
    # Load cached attribution matrix + PMD fit
    ...
    # Use the moderate-sparsity fit (sparse, arm-level). Override via CLI arg.
    K_choice = sys.argv[1] if len(sys.argv) > 1 else "5"
    pmd_fname = f"pmd_K{K_choice}_cv2.87.pkl"
    with open(OUT_DIR / pmd_fname, "rb") as fh:
        pmd = pickle.load(fh)
    res = pmd["res"]
    K = res["K"]
    print(f"loaded PMD fit: {pmd_fname}, K={K}, expl_var={res['explained_var']:.3f}", flush=True)
    ...
    fig, axes = plt.subplots(K, 2, figsize=(8, 2.4 * K),
                                squeeze=False)
    ...
    fig.suptitle("CRC: per-signature direction-reversal test (PFS Kaplan–Meier)",
                  fontsize=10)
