"""
SNV-only Tumor Type Classifier using Variant Features - Ensemble with Macro-Fold Nested CV.

This script trains an ensemble of tumor type classifiers using k-fold cross-validation
with learned variant representations (SNV only).

Features used:
1. Variant features (SNV) - aggregated per sample using weighted average + max pooling

Aggregation Methods:
- weighted_avg: Weighted average + max pooling of features per sample
...
Configuration:
- MODEL_NAME: Name of the variant representation model to load features from
...
Usage:
    python tumor_type_classifier_snv.py
...
# ============================================================================
# Variant model whose features to load from FEATURES_DIR.
MODEL_NAME = os.environ.get("MODEL_NAME", "global_25")
...
# Paths (relative to scripts/tumor_classification_snv/).
FEATURES_DIR = os.environ.get("FEATURES_DIR", "../tcga_pancan_snv/var_features")
...
print("=" * 80)
print("SNV-ONLY TUMOR TYPE CLASSIFIER - ENSEMBLE")
print("=" * 80)
...
# ============================================================================
# 1. Load Variant Features
# ============================================================================
print("\n" + "=" * 60)
print("1. LOADING VARIANT FEATURES")
print("=" * 60)

features_path = os.path.join(FEATURES_DIR, f'TCGA_PanCan_SNV_{MODEL_NAME}_features.pkl')
print(f"Loading features from: {features_path}")

with open(features_path, 'rb') as f:
    features_out = pickle.load(f)

variant_features_train = features_out['train']
variant_features_valid = features_out['valid']

# Load variant data with sample IDs and tumor types
train_data = pd.read_csv(os.path.join(DATA_DIR, 'train_data_snv.csv'))
valid_data = pd.read_csv(os.path.join(DATA_DIR, 'valid_data_snv.csv'))

# Combine train and valid
variant_features = np.vstack([variant_features_train, variant_features_valid])
variant_data = pd.concat([train_data, valid_data], axis=0).reset_index(drop=True)
...
del variant_features_train, variant_features_valid
gc.collect()
...
# Filter variant data
idx_keep = variant_data['type'].isin(valid_tumor_types)
variant_data_filtered = variant_data[idx_keep].reset_index(drop=True)
variant_features_filtered = variant_features[idx_keep.values]
...
# ============================================================================
# 3. Aggregate Variant Features per Sample
# ============================================================================
print("\n" + "=" * 60)
print("3. AGGREGATING VARIANT FEATURES PER SAMPLE")
print("=" * 60)
...
variant_df.index = variant_data_filtered['Sample_ID_Short']
...
print(f"Aggregated variant features: {variant_agg.shape[0]:,} samples")
...
# Get sample IDs
sample_ids_list = list(variant_agg.index)
n_samples = len(sample_ids_list)

print(f"Total samples with aggregated features: {n_samples:,}")
print(f"Feature dimensionality: {variant_agg.shape[1]}")
...
if APPLY_PCA:
    ...
    print(f"\nOriginal dimensionality: {variant_agg.shape[1]}")

    # Standardize features before PCA
    scaler_pca = StandardScaler()
    features_for_pca = scaler_pca.fit_transform(variant_agg.values)
    ...
    final_features = variant_agg.values
...
if USE_MACRO_NESTED_CV:
    ...
    for macro_idx, (macro_train_idx, macro_test_idx) in enumerate(skf_outer.split(X_train, y_train), 1):
        ...
        for inner_idx, (inner_train_idx, inner_val_idx) in enumerate(skf_inner.split(X_macro_train, y_macro_train), 1):
            ...
            model_path = os.path.join(models_dir, f'macro{macro_idx}_inner{inner_idx}_snv{suffix}.keras')
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(X_train, y_train), 1):
        ...
        suffix = '_shuffled' if SHUFFLE_LABELS else ''
        model_path = os.path.join(models_dir, f'snv_fold{fold_idx}{suffix}.keras')
        model.save(model_path)
...
plt.title(f'ROC Curves - SNV Ensemble - {MODEL_NAME}', fontsize=14)
plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=10)
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(plots_dir, f'snv_ensemble_roc_curves_{MODEL_NAME}.png'), dpi=600, bbox_inches='tight')
plt.close()
...
plt.title(f'Precision-Recall Curves - SNV Ensemble - {MODEL_NAME}', fontsize=14)
plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', fontsize=10)
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(plots_dir, f'snv_ensemble_precision_recall_{MODEL_NAME}.png'), dpi=600, bbox_inches='tight')
plt.close()
...
plt.title(f'Confusion Matrix - SNV Ensemble (Normalized) - {MODEL_NAME}', fontsize=16, pad=20)
plt.xlabel('Predicted Label', fontsize=14)
plt.ylabel('True Label', fontsize=14)
plt.xticks(rotation=45, ha='right')
plt.yticks(rotation=0)
plt.tight_layout()
plt.savefig(os.path.join(plots_dir, f'snv_ensemble_confusion_matrix_normalized_{MODEL_NAME}.png'), dpi=300, bbox_inches='tight')
plt.close()
...
plt.title(f'Confusion Matrix - SNV Ensemble (Raw Counts) - {MODEL_NAME}', fontsize=16, pad=20)
plt.xlabel('Predicted Label', fontsize=14)
plt.ylabel('True Label', fontsize=14)
plt.xticks(rotation=45, ha='right')
plt.yticks(rotation=0)
plt.tight_layout()
plt.savefig(os.path.join(plots_dir, f'snv_ensemble_confusion_matrix_counts_{MODEL_NAME}.png'), dpi=300, bbox_inches='tight')
plt.close()
...
# Save label encoder
label_encoder_path = os.path.join(models_dir, f'snv_ensemble_label_encoder_{MODEL_NAME}{suffix}.pkl')
...
results_path = os.path.join(models_dir, f'snv_ensemble_results_{MODEL_NAME}{suffix}.pkl')
...
print("\n" + "=" * 80)
print("SNV-ONLY TUMOR TYPE CLASSIFICATION COMPLETE!")
print("=" * 80)
print(f"\nConfiguration:")
...
print(f"  Features: {n_variant_features} (mean + max pooled + mutational burden)")
