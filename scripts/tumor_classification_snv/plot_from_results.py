"""Per-model plots from saved SNV tumor-type classification results.

Reads a single model's results pkl and emits the per-model figure
panels in Figure 3. Available models: baseline, local_{1,10,25},
global_{1,10,25}.

Inputs
------
``<RESULTS_DIR>/<MODEL>_results.pkl`` (from ``tumor_type_classifier_snv.py``)
...
Usage
-----
    python plot_from_results.py                   # global_25 (default)
    MODEL_NAME=local_25 python plot_from_results.py
"""
...
def plot_from_results(model_name, results_dir=RESULTS_DIR, output_dir=PLOTS_DIR):
    ...
    Args:
        model_name: Name of the SNV model (e.g., 'global_25', 'baseline')
        results_dir: Directory containing results .pkl files
        output_dir: Output directory for plots
    """
    # Construct results file path
    results_path = os.path.join(results_dir, f'snv_ensemble_results_{model_name}.pkl')

    if not os.path.exists(results_path):
        print(f"Error: Results file not found: {results_path}")
        print(f"Available models in {results_dir}:")
        import glob
        for f in glob.glob(os.path.join(results_dir, 'snv_ensemble_results_*.pkl')):
            name = os.path.basename(f).replace('snv_ensemble_results_', '').replace('.pkl', '')
            print(f"  - {name}")
        sys.exit(1)
    ...
    roc_path = os.path.join(output_dir, f'snv_ensemble_roc_curves_{model_name}.png')
    ...
    pr_path = os.path.join(output_dir, f'snv_ensemble_precision_recall_{model_name}.png')
    ...
    cm_norm_path = os.path.join(output_dir, f'snv_ensemble_confusion_matrix_normalized_{model_name}.png')
    ...
    cm_raw_path = os.path.join(output_dir, f'snv_ensemble_confusion_matrix_counts_{model_name}.png')
