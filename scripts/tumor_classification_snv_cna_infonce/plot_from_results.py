"""Per-model plots from saved SNV+CNA InfoNCE classifier results.

Reads the ensemble-results pkl produced by
``tumor_type_classifier_snv_cna_infonce.py`` and emits the per-model
ROC, PR, and confusion-matrix figures (Figure 4 b-c per-class panels).

Inputs
------
``<RESULTS_DIR>/snv_cna_infonce_ensemble_results.pkl``
...
Usage
-----
    python plot_from_results.py                      # LOH variant (default)
...
PLOTS_DIR = os.environ.get("PLOTS_DIR", "plots_macro")
...
# 24 tumor types with COAD+READ combined into COADREAD
# ============================================================================
TUMOR_TYPE_COLORS = {
    'BLCA': '#88CCEE',     # Cyan
    'BRCA': '#44AA99',     # Teal
...
def plot_from_results(results_dir=RESULTS_DIR, output_dir=PLOTS_DIR):
    """
    ...
    results_path = os.path.join(results_dir, 'snv_cna_infonce_ensemble_results.pkl')

    if not os.path.exists(results_path):
        print(f"Error: Results file not found: {results_path}")
        print(f"\nAvailable results files:")
        import glob
        for f in glob.glob(os.path.join(results_dir, '*.pkl')):
    ...
    roc_path = os.path.join(output_dir, 'snv_cna_infonce_ensemble_roc_curves.png')
    ...
    pr_path = os.path.join(output_dir, 'snv_cna_infonce_ensemble_precision_recall.png')
    ...
    plt.title('Confusion Matrix - SNV+CNA InfoNCE Ensemble (Normalized)', fontsize=16, pad=20)
    ...
    cm_norm_path = os.path.join(output_dir, 'snv_cna_infonce_ensemble_confusion_matrix_normalized.png')
    ...
    plt.title('Confusion Matrix - SNV+CNA InfoNCE Ensemble (Raw Counts)', fontsize=16, pad=20)
    plt.xlabel('Predicted Label', fontsize=14)
    plt.ylabel('True Label', fontsize=14)
    plt.xticks(rotation=45, ha='right')
    plt.yticks(rotation=0)
    ...
    cm_raw_path = os.path.join(output_dir, 'snv_cna_infonce_ensemble_confusion_matrix_counts.png')
...
if __name__ == '__main__':
    plot_from_results(RESULTS_DIR, PLOTS_DIR)
