"""Per-model plots from a saved SNV+CNA multimodal classification result.

Reads a single (SNV_MODEL_NAME, CNA_MODEL_NAME) combination's results
pkl and emits the multimodal-classifier figure panel reported in the
manuscript (default global_25 + attn_2).

Inputs
------
``<RESULTS_DIR>/snv_cna_ensemble_results_<SNV>_<CNA>.pkl``
    From ``tumor_type_classifier_snv_cna.py``.

Output
------
``<PLOTS_DIR>/`` -- per-combination figure files.

Usage
-----
    python plot_from_results.py                                   # global_25 + attn_2 (default)
    SNV_MODEL_NAME=local_25 CNA_MODEL_NAME=attn_0 python plot_from_results.py
"""
...
def plot_from_results(snv_model, cna_model, results_dir=RESULTS_DIR, output_dir=PLOTS_DIR):
    """
    Load results from pickle file and generate all plots.

    Args:
        snv_model: Name of the SNV model (e.g., 'global_25', 'baseline')
        cna_model: Name of the CNA model (e.g., 'attn_2', 'attn_0')
        results_dir: Directory containing results .pkl files
        output_dir: Output directory for plots
    """
    # Construct combined model name and results file path
    combined_model_name = f'{snv_model}_{cna_model}'
    results_path = os.path.join(results_dir, f'snv_cna_ensemble_results_{combined_model_name}.pkl')

    if not os.path.exists(results_path):
        print(f"Error: Results file not found: {results_path}")
        print(f"Available model combinations in {results_dir}:")
        import glob
        for f in glob.glob(os.path.join(results_dir, 'snv_cna_ensemble_results_*.pkl')):
            name = os.path.basename(f).replace('snv_cna_ensemble_results_', '').replace('.pkl', '')
            print(f"  - {name}")
        sys.exit(1)
    ...
    print(f"SNV model: {snv_model}")
    print(f"CNA model: {cna_model}")
    ...
    roc_path = os.path.join(output_dir, f'snv_cna_ensemble_roc_curves_{combined_model_name}.png')
    ...
    pr_path = os.path.join(output_dir, f'snv_cna_ensemble_precision_recall_{combined_model_name}.png')
    ...
    cm_norm_path = os.path.join(output_dir, f'snv_cna_ensemble_confusion_matrix_normalized_{combined_model_name}.png')
    ...
    cm_raw_path = os.path.join(output_dir, f'snv_cna_ensemble_confusion_matrix_counts_{combined_model_name}.png')
...
if __name__ == '__main__':
    plot_from_results(SNV_MODEL_NAME, CNA_MODEL_NAME, RESULTS_DIR, PLOTS_DIR)
