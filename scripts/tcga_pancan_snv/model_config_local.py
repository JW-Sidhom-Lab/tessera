"""Local-attention TESSERA SNV model: per-variant flanking-sequence attention only.

Used by ``fit_model.py``, ``get_variant_features.py``,
``get_variant_loss_acc.py``, and ``get_variant_self_attn.py`` via the
``--config local`` CLI flag. ``context_len`` is set on the CLI; the
...
# Local attention only (per-variant context window); no global self-attention.
