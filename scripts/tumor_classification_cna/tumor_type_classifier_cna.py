"""
CNA-only Tumor Type Classifier using CNA Features - Ensemble with Macro-Fold Nested CV.

This script trains an ensemble of tumor type classifiers using k-fold cross-validation
with learned CNA representations (Copy Number Alteration segments only).

Features used:
1. CNA features - aggregated per sample using mean + max pooling

Aggregation Methods:
- mean + max pooling of features per sample (no weighting)

Training Strategies:
1. Macro-Fold Nested CV (USE_MACRO_NESTED_CV=True):
...
2. Full Dataset CV (USE_FULL_DATASET_CV=True, USE_MACRO_NESTED_CV=False):
   - k-fold stratified cross-validation on entire dataset
   - Each fold trains an independent neural network
   - Evaluation on out-of-fold predictions

3. Hold-out Test Set (USE_FULL_DATASET_CV=False, USE_MACRO_NESTED_CV=False):
   - k-fold stratified cross-validation on training data
   - Separate test set held out for final ensemble evaluation

Configuration:
- MODEL_NAME: Name of the CNA model to load features from (attn_0, attn_1, attn_2)
...
Usage:
    python tumor_type_classifier_cna.py
...
# ============================================================================
# CNA model whose features to load from FEATURES_DIR (attn_0, attn_1, attn_2).
...
# Cross-validation mode (only one of these should be True)
...
# Paths (relative to scripts/tumor_classification_cna/).
FEATURES_DIR = os.environ.get("FEATURES_DIR", "../tcga_pancan_cna/cna_features")
...
print("=" * 80)
print("CNA-ONLY TUMOR TYPE CLASSIFIER - ENSEMBLE")
print("=" * 80)
print(f"CNA model: {MODEL_NAME}")
...
# ============================================================================
print("\n" + "=" * 60)
print("1. LOADING CNA FEATURES")
print("=" * 60)

features_path = os.path.join(FEATURES_DIR, f'TCGA_PanCan_CNA_{MODEL_NAME}_cna_features.pkl')
print(f"Loading features from: {features_path}")

with open(features_path, 'rb') as f:
    features_out = pickle.load(f)

# Extract CNA features and metadata
cna_features_train = features_out['train_features_cna']
cna_features_valid = features_out['valid_features_cna']
train_data = features_out['train_data_cna']              # DataFrame with metadata
valid_data = features_out['valid_data_cna']              # DataFrame with metadata

# Combine train and valid
cna_features = np.vstack([cna_features_train, cna_features_valid])
cna_data = pd.concat([train_data, valid_data], axis=0).reset_index(drop=True)
...
# CNA: TCGA-02-0001-01 -> Patient: TCGA-02-0001
...
print(f"  Tumor type mapping: {len(tumor_type_mapping):,} patients")

# Merge tumor types with CNA data
cna_data = cna_data.merge(tumor_type_mapping, on='patient_barcode', how='left')

# Check for missing tumor types
...
del cna_features_train, cna_features_valid, clinical, tumor_type_mapping
gc.collect()
...
# ============================================================================
# 2. Filter for Valid Tumor Types
# ============================================================================
print("\n" + "=" * 60)
print("2. FILTERING TUMOR TYPES")
print("=" * 60)

# Get tumor type counts from CNA data (one row per sample, not per segment)
data_tumor_type = cna_data[['Tumor_Sample_Barcode', 'type']].drop_duplicates()
data_tumor_type_counts = data_tumor_type['type'].value_counts()

print(f"Total unique tumor types in data: {len(data_tumor_type_counts)}")
print(f"Total samples: {len(data_tumor_type):,}")
...
# Filter CNA data
idx_keep = cna_data['type'].isin(valid_tumor_types)
cna_data_filtered = cna_data[idx_keep].reset_index(drop=True)
cna_features_filtered = cna_features[idx_keep.values]

# Extract short sample ID (first 15 chars) for aggregation
cna_data_filtered['Sample_ID_Short'] = cna_data_filtered['Tumor_Sample_Barcode'].str[:15]

print(f"\nFiltered to {len(valid_tumor_types)} tumor types")
print(f"CNA segment features after filtering: {cna_features_filtered.shape[0]:,}")
...
# ============================================================================
# 3. Aggregate CNA Features per Sample
# ============================================================================
print("\n" + "=" * 60)
print("3. AGGREGATING CNA FEATURES PER SAMPLE")
print("=" * 60)
...
cna_agg_mean = cna_df.groupby(cna_df.index).apply(simple_mean)

# Compute max pooled features
print("  Computing max pooled features...")
cna_agg_max = cna_df.groupby(cna_df.index).apply(max_pooling)
...
segment_count = cna_df.groupby(cna_df.index).size()
cna_agg['segment_count'] = np.log1p(segment_count)
...
print(f"  Mean segment count: {segment_count.mean():.1f} segments/sample")
print(f"  Median segment count: {segment_count.median():.1f} segments/sample")
print(f"  Range: {segment_count.min()}-{segment_count.max()} segments/sample")

# Clean up intermediate aggregation variables
print("  Cleaning up aggregation intermediates...")
del cna_features_scaled, cna_df, cna_agg_mean, cna_agg_max, segment_count
gc.collect()
...
# ============================================================================
print("\n" + "=" * 60)
print("4. PREPARING FINAL FEATURE MATRIX")
print("=" * 60)

# Get sample IDs
sample_ids_list = list(cna_agg.index)
n_samples = len(sample_ids_list)

print(f"Total samples with aggregated features: {n_samples:,}")
print(f"Feature dimensionality: {cna_agg.shape[1]}")


# ============================================================================
# 4b. Optional PCA on Aggregated Features
# ============================================================================
if APPLY_PCA:
    print("\n" + "=" * 60)
    print("4b. APPLYING PCA TO AGGREGATED FEATURES")
    print("=" * 60)

    print(f"\nOriginal dimensionality: {cna_agg.shape[1]}")

    # Standardize features before PCA
    scaler_pca = StandardScaler()
    features_for_pca = scaler_pca.fit_transform(cna_agg.values)
...
# ============================================================================
print("\n" + "=" * 60)
print("5. PREPARING LABELS")
print("=" * 60)

# Get tumor types for samples using truncated IDs
tumor_type_map = cna_data_filtered[['Sample_ID_Short', 'type']].drop_duplicates()
...
# ============================================================================
# 6. Train/Test Split (or use full dataset for CV)
# ============================================================================
print("\n" + "=" * 60)
...
def create_mlp_model(input_dim, n_classes, dropout_rate=0.5):
    """Create MLP classifier for variant features"""
...
del cna_data, cna_data_filtered, cna_agg
...
if USE_MACRO_NESTED_CV:
    ...
    for macro_idx, (macro_train_idx, macro_test_idx) in enumerate(skf_outer.split(X_train, y_train), 1):
        ...
        for inner_idx, (inner_train_idx, inner_val_idx) in enumerate(skf_inner.split(X_macro_train, y_macro_train), 1):
            ...
            model_path = os.path.join(models_dir, f'macro{macro_idx}_inner{inner_idx}_cna{suffix}.keras')
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(X_train, y_train), 1):
        ...
        suffix = '_shuffled' if SHUFFLE_LABELS else ''
        model_path = os.path.join(models_dir, f'cna_fold{fold_idx}{suffix}.keras')
        model.save(model_path)
...
print("\n" + "=" * 60)
if USE_MACRO_NESTED_CV:
    print("8. EVALUATING MACRO-FOLD NESTED CV RESULTS")
elif USE_FULL_DATASET_CV:
    print("8. EVALUATING ON OUT-OF-FOLD PREDICTIONS")
else:
    print("8. EVALUATING ENSEMBLE ON TEST SET")
print("=" * 60)
...
colors = [get_tumor_color(label_encoder.classes_[i], i) for i in all_class_indices]
...
plt.tight_layout()
plt.savefig(os.path.join(plots_dir, f'cna_ensemble_roc_curves_{MODEL_NAME}.png'), dpi=600, bbox_inches='tight')
plt.close()
...
# ============================================================================
print("\n" + "=" * 60)
print("10. PRECISION-RECALL ANALYSIS")
print("=" * 60)
...
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(plots_dir, f'cna_ensemble_precision_recall_{MODEL_NAME}.png'), dpi=600, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(plots_dir, f'cna_ensemble_confusion_matrix_normalized_{MODEL_NAME}.png'), dpi=300, bbox_inches='tight')
plt.close()
...
plt.yticks(rotation=0)
plt.tight_layout()
plt.savefig(os.path.join(plots_dir, f'cna_ensemble_confusion_matrix_counts_{MODEL_NAME}.png'), dpi=300, bbox_inches='tight')
plt.close()
...
# Save label encoder
label_encoder_path = os.path.join(models_dir, f'cna_ensemble_label_encoder_{MODEL_NAME}{suffix}.pkl')
...
results_path = os.path.join(models_dir, f'cna_ensemble_results_{MODEL_NAME}{suffix}.pkl')
...
print("\n" + "=" * 80)
print("CNA-ONLY TUMOR TYPE CLASSIFICATION COMPLETE!")
print("=" * 80)
print(f"\nConfiguration:")
...
print(f"  Features: {n_cna_features} (mean + max pooled + segment count)")
